...
describe('Command Security Tests', () => {
  ...
  describe('Command Integration Tests', () => {
    test('should handle init command safely', async () => {
      ...
      // Should complete without throwing - init is safer than check
      try {
        await initCommand({});
        expect(true).toBe(true); // Success
      } catch (error) {
        // Error is acceptable as long as it's not a security issue
        expect(error.message).not.toMatch(/injection|xss|path traversal/i);
      }
    ...
    test('should handle check command safely', async () => {
      ...
      try {
        await checkCommand({});
        // Should complete successfully or call process.exit
        expect(exitCalled || true).toBe(true);
      } catch (error) {
        // Error is acceptable as long as it's not a security issue
        expect(error.message).not.toMatch(/injection|xss|path traversal/i);
      } finally {
